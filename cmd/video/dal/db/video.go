package db

import (
	"context"
	"fmt"
	"gorm.io/gorm"
	"miniTikTok/pkg/constants"
	"miniTikTok/pkg/errno"
)

type Video struct {
	gorm.Model
	PlayUrl       string `gorm:"column:play_url;type:varchar(255);not null"`
	CoverUrl      string `gorm:"column:cover_url;type:varchar(255);not null"`
	FavoriteCount int64  `gorm:"column:favorite_count;type:bigint;not null"`
	CommentCount  int64  `gorm:"column:comment_count;type:bigint;not null"`
	Title         string `gorm:"column:title;type:varchar(255);not null"`
	AuthorId      int64  `gorm:"column:author_id;type:bigint;not null"`
}

func (v *Video) TableName() string {
	return constants.VideoTableName
}

// QueryVideoById query video info by author id
func QueryVideoById(ctx context.Context, userId int64) ([]*Video, error) {
	res := make([]*Video, 0)
	if err := DB.WithContext(ctx).Where("author_id = ?", fmt.Sprint(userId)).Find(&res).Error; err != nil {
		return []*Video{}, err
	}
	return res, nil
}

// PublishVideo publish video use userid
func PublishVideo(ctx context.Context, v *Video) error {
	var videos []*Video = make([]*Video, 1)
	videos[0] = v
	if DB.Create(videos).Error != nil {
		return errno.InsertErr
	}
	return nil
}
