...
// AddCommentById add video favorite count by vedio id
func AddCommentById(ctx context.Context, videoId int64) error {
	video := Video{}
	video.ID = uint(videoId)
	//Favorite Count + 1
	if err := DB.WithContext(ctx).Model(&video).Update("comment_count", gorm.Expr("comment_count + ?", 1)).Error; err != nil {
		return err
	}
	return nil
}

// DecCommentById decrease video favorite count by vedio id
func DecCommentById(ctx context.Context, videoId int64) error {
	video := Video{}
	video.ID = uint(videoId)
